...
public class FourOfAKind {
		...
		while(true)
		{
			if(curPlayer == HUMAN)
			{
				...
				if(isFourOfAKind(hCards))
				{
					...
					return; //Exit application by return from main()
				}
				curPlayer = COMPURTER;
			}
			else 
	...
	static int leastDesirableCard(Card[] cards) 
	{
		...
		return 0;//Needed to satisfy compiler
	}
	...
	static int prompt(String msg) 
	{
		...
		try {
			...
			//Erase all subsequent charachers including termining \n newline
